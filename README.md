...
sequenceDiagram
    ...
    API->>Detector: verify_claim()
    
    Detector->>Agents: Classify Claim
    Agents-->>Detector: Classification Result
    
    Detector->>Agents: Decompose Claim
    Agents-->>Detector: Atomic Claims
    
    loop For Each Atomic Claim
        Detector->>Agents: Generate Queries
        Agents-->>Detector: 5 Search Queries
        
        par Parallel Search Execution
            Detector->>Perplexity: Query 1
            Detector->>Perplexity: Query 2
            Detector->>Perplexity: Query 3
            Detector->>Perplexity: Query 4
            Detector->>Perplexity: Query 5
        end
        
        Perplexity-->>Detector: Responses with Citations
        
        par Parallel Classification
            Detector->>Claude: Classify Response 1
            Detector->>Claude: Classify Response 2
            Detector->>Claude: Classify Response 3
            Detector->>Claude: Classify Response 4
            Detector->>Claude: Classify Response 5
        end
        
        Claude-->>Detector: Classifications (SUPPORT/CONTRADICT/IRRELEVANT)
        
        Detector->>Scorer: Calculate Trust Scores
        Scorer-->>Detector: Trust Scores
    end
    
    Detector->>Detector: Aggregate Evidence
    Detector->>Detector: Calculate Verdict
    Detector-->>API: Verification Result
